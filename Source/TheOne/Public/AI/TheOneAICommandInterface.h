// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "Types/TheOneUseAbilityCommandType.h"
#include "UObject/Interface.h"
#include "TheOneAICommandInterface.generated.h"

// This class does not need to be modified.
UINTERFACE(MinimalAPI)
class UTheOneAICommandInterface : public UInterface
{
	GENERATED_BODY()
};

/**
 * 
 */
class THEONE_API ITheOneAICommandInterface
{
	GENERATED_BODY()

	// Add interface functions to this class. This is the class that will be inherited to implement this interface.
public:
	UFUNCTION(BlueprintImplementableEvent, BlueprintCallable)
	void SetAITargetLocation(const FVector& Location);
	
	UFUNCTION(BlueprintImplementableEvent, BlueprintCallable)
	void SetAITargetActor(AActor* TargetActor);

	UFUNCTION(BlueprintImplementableEvent, BlueprintCallable)
	void CommitAbility(ETheOneUseAbilityCommandType InUseAbilityCommandType, int32 InIntPayload, AActor* InTargetActor, const FVector& InLocation, int InReleaseDistance);
};
