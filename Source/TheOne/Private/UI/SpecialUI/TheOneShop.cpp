...
void UTheOneShop::RefreshTreasure()
{
	...
	// 临时： 刷新出全部的武器， 创建武器实例
	auto WeaponDT = GetDefault<UTheOneDataTableSettings>()->WeaponTable.LoadSynchronous();
	...
	for (const auto& Elem : RowMap)
	{
		...
		ItemSystem->CreateItemInstance(Elem.Key, ETheOneItemType::Weapon, TreasureLogicSlots[Count]);
		Count++;
	}
}
